...
setActiveID(id)
  ...
  useEffect(() => {
    ...
    if (selectTap === 'mobile-app') {
      const filteredData = data.filter(item => item.category === 'Mobile App');
      setPortfolios(filteredData);
    }
  }, [selectTap]);
      ...
      <div className="container">
        <div className="flex items-center justify-between flex-wrap m-5">
          ...
          <div  className="flex gap-3">
            <button className="text-smallTextColor border border-solid border-smallTextColor py-2 px-4 rounded-[8px]"onClick={() => setSelectTab('all')}>
              All
            </button>
            <button
